...
class BotThings(TwitterBot):
    ...
    def on_mention(self, tweet, prefix):
        ...
        if diff.seconds <= 120:
            if tweet.text.startswith("@bot_things BUTT "):
                # get the word butt_things tweeted
                butt_text = tweet.text.lower()
                butt_text = re.sub(r".+\s+butt", "", butt_text)

                text = random.choice((' BOT ', ' BOTT ')) + butt_text.strip()
